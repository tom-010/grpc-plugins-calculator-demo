...
impl Calculator for MyCalculator {
    async fn calculate(
        ...
        if rand::random::<f64>() > 0.9 {
            println!("[multiply-plugin] Ahh! I'm crashing!");
            panic!("A random, unhandled error occurred!");
        }

        let result = req.a * req.b;
        println!(
            "[multiply-plugin] Calculated {} * {} = {}",
            req.a, req.b, result
        );
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let socket_path = std::env::args().nth(1).expect("Usage: multiply <socket_path>");
    ...
    println!("[multiply-plugin] Server listening on socket: {}", socket_path);
