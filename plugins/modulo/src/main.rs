...
impl Calculator for MyCalculator {
    async fn calculate(
        ...
        if rand::random::<f64>() > 0.9 {
            println!("[modulo-plugin] Ahh! I'm crashing!");
            panic!("A random, unhandled error occurred!");
        }

        let result = req.a % req.b;
        println!(
            "[modulo-plugin] Calculated {} % {} = {}",
            req.a, req.b, result
        );
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let socket_path = std::env::args().nth(1).expect("Usage: modulo <socket_path>");
    ...
    println!("[modulo-plugin] Server listening on socket: {}", socket_path);
